...
### ✅ Stage 0 代码实现状态（2026-04-17 完成）

1. ✅ **部分监督机制已实现**：`generate_supervision_mask()`、`is_labeled` 字段、masked MSE loss
2. ✅ **物理约束 `forward()` 支持 supervision_mask**：无标签样本参与物理约束，不参与 MSE
3. ✅ **批量运行脚本 `run_baseline_v22.py`** 就绪，支持断点续跑
4. ✅ **烟雾测试通过**（ratio=1.0 MAE 1.70%，ratio=0.5 MAE 1.75%，5 epochs）
5. ⚠️ **`cnn_lstm_config.json` 中 `enabled` 当前为 `false`**（用户手动调整）——运行基线前需确认开关状态
6. ℹ️ **20 次实验（5 seeds × 4 ratios）尚未运行**，下一步执行 `python run_baseline_v22.py`
