...
% APPLY_WINDOWING_WITH_METADATA 应用滑动窗口并附加metadata
% 对应Python: data_loaders/data_loader_hust.py::apply_windowing_with_metadata
%
% 输入:
%   features: (total_cycles, feature_dim) 特征矩阵
%   targets: (total_cycles, 1) 目标值向量
%   window_size: 窗口大小
...
%   cycle_indices: (N, 1) cycle索引
    ...
    % 创建滑动窗口（对应Python的for i in range）
    for i = 1:num_windows
        % 输入窗口（对应Python: features[i:i+window_size]）
        X(i, :, :) = features(i:i+window_size-1, :);

        if strcmp(mode, 'many_to_one')
            % Many-to-One: 预测窗口最后一个点的目标值
            % 对应Python: y_windows.append(targets[i+window_size-1])
        ...
        elseif strcmp(mode, 'many_to_many')
            ...
            % 对应Python: y_windows.append(targets[i:i+window_size])
