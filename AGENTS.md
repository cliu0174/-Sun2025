# PI-CNNLSTM 电池 SOH 估计项目 — Codex 记忆卡

> 本文件在新会话中会自动加载。阅读后请优先查阅下方详细计划，避免重复讨论已确定的方案。

## 项目一句话概述

基于**物理一致性约束**的电池 SOH 估计（PI-CNNLSTM），核心场景是**部分生命周期监督**（Partial Lifecycle Supervision）——即训练电池内仅有部分循环级 SOH 标签。数据集为 HUST 77 块电池，当前正式实现和正文统一使用 16 维充电特征。
...
- **当前阶段**：**Springer 单栏正文和泄漏隔离主实验管线已完成；正式多种子矩阵待运行/正在运行**
- **代码现状**：
...
- **下一步行动**：主表采用固定 split seed 42，在 `[929, 2262, 7]` 三个训练种子与独立 mask seeds 下配对重复；多个独立电池划分只作敏感性补充，不与主表混合平均。结果归档后生成 600 dpi 图表并写入 Springer 主稿第 4 节。
