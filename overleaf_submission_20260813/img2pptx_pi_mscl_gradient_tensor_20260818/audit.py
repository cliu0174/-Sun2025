...
def main():
    ...
    checks["cnn.slanted_multicolor_grids"] = len(cnn_polygons) >= 200 and all(g is not None for g in gradient_groups)
    ...
    for gradient_group in gradient_groups:
        ...
        ordered_gradients &= ranks == list(range(18)) and all(b > a for a, b in zip(luminance, luminance[1:]))
