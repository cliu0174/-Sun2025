...
OUTPUT_PPTX = ROOT / "PI-MSCL_architecture_native_shapes_conv_cells_tighter_20260818.pptx"
OUTPUT_PPTX_UNGROUPED = ROOT / "PI-MSCL_architecture_native_shapes_fully_ungrouped_conv_cells_tighter_20260818.pptx"
...
STACK_BORDER_LIGHTEN = {
    "2E6DA4": 0.32,  # Conv1D blue
    "398C43": 0.32,  # Conv1D green
    "7251A7": 0.32,  # Conv1D purple
    "6D8EA7": 0.28,  # fused tensor top facet
    "557994": 0.28,  # fused tensor side/back layers
    "3F6F93": 0.28,  # fused tensor front outline
}
...
def blend_hex_with_white(value: str, fraction: float) -> str:
...
def apply_line(shape, element: ET.Element) -> None:
    ...
    if stroke == "none":
        ...
        return
    stroke_key = stroke.lstrip("#").upper()
    display_stroke = (
        blend_hex_with_white(stroke, STACK_BORDER_LIGHTEN[stroke_key])
        if stroke_key in STACK_BORDER_LIGHTEN
        else stroke
    )
    shape.line.color.rgb = rgb(display_stroke)
    width_px = float(element.attrib.get("stroke-width", "1"))
    # Colored matrix cells use a white stroke to create their gutters. Reduce
    # that stroke slightly so neighboring cells sit closer while retaining a
    # clean visible separator at normal slide scale.
    min_width_pt = 0.10
    if stroke_key == "FFFFFF":
        if element.attrib.get("data-gradient-rank") is not None:
            # Conv1D front grids share exact cell boundaries, so their visible
            # whitespace comes from the centered white stroke. Make these 126
            # cell separators distinctly finer without changing Input or the
            # fused tensor spacing already approved in the prior revision.
            width_px *= 0.22
            min_width_pt = 0.06
        else:
            width_px *= 0.42
    shape.line.width = Pt(max(min_width_pt, width_px * PX_TO_PT))
    shape.line.dash_style = dash_style(element.attrib.get("stroke-dasharray"))
    # SVG element opacity applies to both fill and stroke. The first native
    # conversion applied it only to the fill, making the rear plates of the
    # Conv1D/tensor stacks look too dark. Mirror the same layer opacity onto
    # the PowerPoint line color so rear borders fade with their plates.
    solid_fill = shape._element.spPr.ln.solidFill
    if solid_fill is not None:
        line_opacity = opacity_of(element)
        if element.attrib.get("data-gradient-rank") is not None:
            # PowerPoint rasterizes extremely thin strokes to roughly one
            # screen pixel. Lower the separator alpha as well so the remaining
            # hairline reads as a finer gutter instead of a full white gap.
            line_opacity = min(line_opacity, 0.48)
        set_alpha_on_solid_fill(solid_fill, line_opacity)
...
def add_rect(tree, element: ET.Element, name: str):
    ...
    # Input uses a 19 px pitch with 17 px squares, leaving a 2 px geometric
    # gutter. Enlarge only these colored cells around their centers so the
    # actual gap becomes 0.8 px, matching the tighter Conv1D/tensor grids.
    if (
        "input_sequence" in name
        and element.attrib.get("stroke", "").upper() == "#FFFFFF"
        and element.attrib.get("fill", "").upper() != "#FFFFFF"
        and abs(w - 17.0) < 0.01
        and abs(h - 17.0) < 0.01
    ):
        x -= 0.6
