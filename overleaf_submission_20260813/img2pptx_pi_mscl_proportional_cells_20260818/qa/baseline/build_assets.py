...
def matrix_module():
    ...
    palette = ["#8FB9DE", "#9BC1DF", "#A9C9E2", "#9FC7BA", "#ACCFBF", "#BBD8C9"]
    for r in range(8):
        for col in range(6):
            color = palette[col]
            if r >= 4 and col < 3:
                color = ["#7B9FD1", "#86AAD2", "#91B4D8"][col]
            c.append(rect(78 + 18*col, 155 + 19*r, 15, 17, fill=color, stroke="#FFFFFF", sw=0.8))
    ...
    for r in range(2):
        for col, color in enumerate(tail_colors):
            c.append(rect(78 + 18*col, 334 + 19*r, 15, 17, fill=color, stroke="#FFFFFF", sw=0.8))
...
def feature_stack(prefix, x, y, w, h, fill, stroke):
    ...
    for i in range(5):
        off = 4*(4-i)
        c.append(rect(x-off, y+off, w, h, fill=fill, stroke=stroke, sw=0.8, extra=f'opacity="{0.38+0.11*i:.2f}"'))
    for k in range(1, 6):
        c.append(line(x + w*k/6, y, x + w*k/6, y+h, stroke=stroke, sw=0.45))
    for k in range(1, 3):
        c.append(line(x, y + h*k/3, x+w, y + h*k/3, stroke=stroke, sw=0.45))
...
def output_module():
    ...
    c.extend([line(1525,350,1648,350,stroke="#333333",sw=1.1), line(1525,199,1525,350,stroke="#333333",sw=1.1)])
    for yy, val in [(204,"1.0"),(233,"0.8"),(263,"0.6"),(292,"0.4"),(321,"0.2"),(350,"0.0")]:
        c.append(line(1525,yy,1648,yy,stroke="#C8C8C8",sw=0.8,dash="4 4")); c.append(text(1517,yy+4,val,10,"end"))
    pts=[(1536,209),(1550,219),(1566,232),(1578,250),(1588,274),(1601,298),(1615,313),(1628,323),(1642,340)]
    c.append(polyline(pts,stroke="#2F7D22",sw=2.0))
    for x,y in pts: c.append(circle(x,y,4,fill="#5CAB3E",stroke="#1E5F1A",sw=0.9))
    c.extend([text(1536,368,"1",10,"middle"),text(1642,368,"10",10,"middle"),text(1588,392,"cycle index",11,"middle")])
...
def model_connectors():
    return group("model_connectors", [
        ...
        arrow([(882,263),(922,263)], sw=1.6, head=8), arrow([(976,263),(1016,263)], sw=1.6, head=8), arrow([(1318,260),(1337,260)], sw=1.6, head=8), arrow([(1467,260),(1498,260)], sw=1.6, head=8)
    ])
